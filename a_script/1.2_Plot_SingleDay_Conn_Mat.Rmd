...
nc <- nc_open(
  here::here("../HYCOM-Galapagos/galapagos_2013-08-21_.nc")) 
# choose your path

if(length(which(abs(nc$var$water_u$dim[[1]]$vals)>180))>=1){
  
  rr_u <- raster::rotate(raster(
  here::here("../HYCOM-Galapagos/galapagos_2013-08-21_.nc"), 
    level = 1, varname = "water_u", band = 1))
  rr_v <- raster::rotate(raster(
  here::here("../HYCOM-Galapagos/galapagos_2013-08-21_.nc"), 
...
png(filename = here::here("c_output/figures/sea_v_u_gal.png"), width = 800, height = 600)

# Set new extent
new_extent <- extent(-92.3, -88.6, -1.8, 0.92)
extent(sea_spe) <- new_extent

# Plot the raster with sea current speed and direction
plot(sea_spe, main = "Sea current speed and direction in Galapagos", col = acol(1000), xlab = "Longitude", ylab = "Latitude")

# Add arrowheads for direction
Arrowhead(coordsraster[,1], coordsraster[,2], angle = arrowDir(dirspeed), arr.length = 0.05, arr.type = "curved")
...
# loeas cost paths based on conductance matrices - 1 day
...
files_Galap_1Day <- list.files(
  here::here("c_output/one_day_path_plot"),#choose your local path
...
cost_list_1dayplot_Gal<-calculate_costMatrix(files_Galap_1Day, Galapagos_points)
...
AtoB<- shortestPath(conductance_1dayplot_Gal[[1]], #I´m using the raster of the first day 
                    #as an example here
                    pntA, pntB, 
                    output = "SpatialLines")
BtoA<- shortestPath(conductance_1dayplot_Gal[[1]], 
                    pntB, 
                    pntA, 
                    output = "SpatialLines")
...
velocity_gal<-image.plot(daystacklist_1dayplot_Gal[[1]]$speed,
           main = "least cost paths by current speed",
           col = terrain.colors(10), xlab = "Longitude", ylab = "Lattitude",
           zlim = c(0.01, 0.999))

lines(getMap(resolution = "high"), lwd = 2)    # adds lines around islands

points(pntA, pch = 19, cex = 3.4, col = "red")   #adds pntA
points(pntB, pch = 19, cex = 3.4, col = "blue")  #adds pntB

lines(AtoB, col = "red", lwd = 2, lty = 2)   #draws line a to b
lines(BtoA, col = "blue", lwd = 2, lty = 2)  #b to a
...
direction_gal<-image.plot(daystacklist_1dayplot_Gal[[1]]$direction,    # background plot of speed raster for 
