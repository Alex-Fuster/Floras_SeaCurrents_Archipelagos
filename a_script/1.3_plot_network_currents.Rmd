...
## compute % thalassochorous and % littoral species in floras
...
for (island in island_cols) {
  ...
  # Calculate the percentage of thalassochorous species
  thalassochorous_species <- species_in_island[species_in_island$thalassochory %in% c(0.5, 1), ]
  percent_thalassochorous <- (nrow(thalassochorous_species) / total_species) * 100
...
for (island in island_cols) {
  ...
  # Calculate the percentage of thalassochorous species
  thalassochorous_species <- species_in_island[species_in_island$THA %in% c("0.5", "1", "0.33"), ]
  percent_thalassochorous <- (nrow(thalassochorous_species) / total_species) * 100
...
Raw plots % tha and % littoral


```{r}


# Convert df_percent_flora_az to long format for ggplot
df_percent_flora_az_long <- reshape2::melt(df_percent_flora_az, id.vars = c("Island", "Total_Species"), 
                                           variable.name = "Species_Type", 
                                           value.name = "Percentage")

# Create the barplot
ggplot(df_percent_flora_az_long, aes(x = Island, y = Percentage, fill = Species_Type)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9), width = 0.7) +
  geom_text(data = df_percent_flora_az_long, aes(x = Island, y = max(df_percent_flora_az_long$Percentage) + 2, 
                                            label =  Total_Species),
...
compute_df_percent_flora_degree <- function(mat_min_conn_currents, df_percent) {
  ...
  node_list <- merge(node_list, df_percent[,c("Island", "Percent_Littoral", "Percent_Thalassochorous")], by.x = "name", by.y = "Island", all.x = TRUE)
  ...
  # Authorities (sinks)
  V(tg)$authorities <- authority.score(tg, weights = E(tg)$inv_weight)$vector
...
```{r}
cor_centrality_gal <- cor(df_percent_degree_all[which(df_percent_degree_all$archipelago == "Galapagos"), c("in_degree", "out_degree", "closeness", "hubs", "authorities")])

cor_centrality_can <- cor(df_percent_degree_all[which(df_percent_degree_all$archipelago == "Canaries"), c("in_degree", "out_degree", "closeness", "hubs", "authorities")])

cor_centrality_az <- cor(df_percent_degree_all[which(df_percent_degree_all$archipelago == "Azores"), c("in_degree", "out_degree", "closeness", "hubs", "authorities")])


plot_cor_centrality_measures <- ggpairs(df_percent_degree_all[,-c(1:3)], aes(color = archipelago),
...
## Plot correlations % flora and centrality measures
...
plot_litt_degree_gal <- ggplot(df_percent_degree_all[df_percent_degree_all$archipelago == "Galapagos",], aes(x = in_degree, y = Percent_Littoral)) +
  geom_point(size = 3, color = "#1f78b4") + # Add points
  geom_smooth(method = "lm", se = FALSE, color = "#1f78b4") + # Add trend lines
  labs(x = "In-degree centrality from\n sea currents network", y = "% littoral plants in lowland") +
...
plot_litt_authority_gal <- ggplot(df_percent_degree_all[df_percent_degree_all$archipelago == "Galapagos",], aes(x = log(authorities), y = Percent_Littoral)) +
  ...
  labs(x = "Authority centrality from\n sea currents network", y = "% littoral plants in lowland") +
  ylim(0,max_perc_litt+1)+
  theme_classic() + # Use a minimal theme
  my_theme
...
plot_litt_degree_can <- ggplot(df_percent_degree_all[df_percent_degree_all$archipelago == "Canaries",], aes(x = log(in_degree), y = Percent_Littoral)) +
  geom_point(size = 3, color = "#33a02c") + # Add points
  geom_smooth(method = "lm", se = FALSE, color = "#33a02c") + # Add trend lines
   scale_x_continuous(labels = function(x) sprintf("%.2f", back_transform_log(x))) +
  labs(x = "In-degree centrality from\n sea currents network", y = "% littoral plants in lowland") +
  ylim(0,max_perc_litt+1)+
  theme_classic() + # Use a minimal theme
...
plot_litt_degree_az <- ggplot(df_percent_degree_all[df_percent_degree_all$archipelago == "Azores",], aes(x = in_degree, y = Percent_Littoral)) +
  geom_point(size = 3, color = "#e31a1c") + # Add points
  geom_smooth(method = "lm", se = FALSE, color = "#e31a1c") + # Add trend lines
  labs(x = "In-degree centrality from\n sea currents network", y = "% littoral plants in lowland") +
  ylim(0,max_perc_litt+1)+
  theme_classic() + # Use a minimal theme
  my_theme
...
% Thalassochory & centrality

```{r}

# maximum %

max_perc_tha <- max(df_percent_degree_all$Percent_Thalassochorous)


# Galapagos 

plot_tha_degree_gal <- ggplot(df_percent_degree_all[df_percent_degree_all$archipelago == "Galapagos",], aes(x = in_degree, y = Percent_Thalassochorous)) +
  geom_point(size = 3, color = "#1f78b4") + # Add points
  geom_smooth(method = "lm", se = FALSE, color = "#1f78b4") + # Add trend lines
  labs(x = "In-degree centrality from\n sea currents network", y = "% thalassochorous plants in lowland") +
  ylim(0,max_perc_tha+1)+
  theme_classic() + # Use a minimal theme
  my_theme


plot_tha_authority_gal <- ggplot(df_percent_degree_all[df_percent_degree_all$archipelago == "Galapagos",], aes(x = log(authorities), y = Percent_Thalassochorous)) +
  geom_point(size = 3, color = "#1f78b4") + # Add points
  geom_smooth(method = "lm", se = FALSE, color = "#1f78b4") + # Add trend lines
      scale_x_continuous(labels = function(x) sprintf("%.2f", back_transform_log(x))) +
  labs(x = "Authority centrality from\n sea currents network", y = "% thalassochorous plants in lowland") +
...
plot_tha_degree_can <- ggplot(df_percent_degree_all[df_percent_degree_all$archipelago == "Canaries",], aes(x = log(in_degree), y = Percent_Thalassochorous)) +
  geom_point(size = 3, color = "#33a02c") + # Add points
  geom_smooth(method = "lm", se = FALSE, color = "#33a02c") + # Add trend lines
   scale_x_continuous(labels = function(x) sprintf("%.2f", back_transform_log(x))) +
  labs(x = "In-degree centrality from\n sea currents network", y = "% thalassochorous plants in lowland") +
...
plot_tha_authority_can <- ggplot(df_percent_degree_all[df_percent_degree_all$archipelago == "Canaries",], aes(x = log(authorities), y = Percent_Thalassochorous)) +
  geom_point(size = 3, color = "#33a02c") + # Add points
  geom_smooth(method = "lm", se = FALSE, color = "#33a02c") + # Add trend lines
      scale_x_continuous(labels = function(x) sprintf("%.3f", back_transform_log(x))) +
  labs(x = "Authority centrality from\n sea currents network", y = "% thalassochorous plants in lowland") +
...
plot_tha_degree_az <- ggplot(df_percent_degree_all[df_percent_degree_all$archipelago == "Azores",], aes(x = in_degree, y = Percent_Thalassochorous)) +
  geom_point(size = 3, color = "#e31a1c") + # Add points
  geom_smooth(method = "lm", se = FALSE, color = "#e31a1c") + # Add trend lines
  labs(x = "In-degree centrality from\n sea currents network", y = "% thalassochorous plants in lowland") +
  ylim(0,max_perc_tha+1)+
...
plot_tha_authority_az <- ggplot(df_percent_degree_all[df_percent_degree_all$archipelago == "Azores",], aes(x = log(authorities), y = Percent_Thalassochorous)) +
  geom_point(size = 3, color = "#e31a1c") + # Add points
  geom_smooth(method = "lm", se = FALSE, color = "#e31a1c") + # Add trend lines
      scale_x_continuous(labels = function(x) sprintf("%.2f", back_transform_log(x))) +
  labs(x = "Authority centrality from\n sea currents network", y = "% thalassochorous plants in lowland") +
...
plot_perc_tha_centrality <- ggarrange(
  plot_tha_degree_gal+ labs(title = ""), plot_tha_degree_can + ylab(NULL)+ labs(title = ""), plot_tha_degree_az + ylab(NULL)+ labs(title = ""),
  plot_tha_authority_gal+ labs(title = ""), plot_tha_authority_can + ylab(NULL)+ labs(title = ""), plot_tha_authority_az + ylab(NULL)+ labs(title = ""),
...
plot_perc_tha_centrality

#ggsave(here::here("c_output/figures/plot_perc_tha_centrality.png"), height = 9, width = 9)
...
plot_network_littoral(weighted_matrix = mat_medianMin_sym_Galapagos,
...
plot_network_littoral(weighted_matrix = mat_medianMin_sym_Canaries,
...
plot_network_littoral(weighted_matrix = mat_medianMin_sym_Azores,
...
plot_network_in_degree(weighted_matrix = mat_medianMin_sym_Galapagos, 
                       centroids = coord_centroids_gal, 
...
plot_network_in_degree(weighted_matrix = mat_medianMin_sym_Canaries, 
                       centroids = coord_centroids_can, 
...
plot_network_in_degree(weighted_matrix = mat_medianMin_sym_Azores, 
                       centroids = coord_centroids_az, 
