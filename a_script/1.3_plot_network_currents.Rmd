...
# Reorder Island factor based on Percentage in descending order
df_percent_flora_gal_long$Island <- factor(df_percent_flora_gal_long$Island, 
                                           levels = df_percent_flora_gal_long$Island[order(df_percent_flora_gal_long$Percentage, decreasing = TRUE)])

df_percent_flora_can_long$Island <- factor(df_percent_flora_can_long$Island, 
                                           levels = df_percent_flora_can_long$Island[order(df_percent_flora_can_long$Percentage, decreasing = TRUE)])

df_percent_flora_az_long$Island <- factor(df_percent_flora_az_long$Island, 
                                          levels = df_percent_flora_az_long$Island[order(df_percent_flora_az_long$Percentage, decreasing = TRUE)])


ymax = max(c(df_percent_flora_gal_long$Percentage, df_percent_flora_can_long$Percentage, df_percent_flora_az_long$Percentage))


# Create plots with reordered dataframes
plot_gal <- ggplot(df_percent_flora_gal_long, aes(x = Island, y = Percentage)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9), width = 0.7, fill = "#1f78b4") +
  geom_text(data = df_percent_flora_gal_long, aes(x = Island, y = ymax + 0.5, 
                                            label =  Total_Species),
...
plot_can <- ggplot(df_percent_flora_can_long, aes(x = Island, y = Percentage)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9), width = 0.7, fill = "#33a02c") +
  geom_text(data = df_percent_flora_can_long, aes(x = Island, y = ymax + 0.5, 
                                            label =  Total_Species),
            color = "black", size = 4, vjust = -0.5) +
  labs(x = NULL,
       y = "% littoral species") +
...
plot_az <- ggplot(df_percent_flora_az_long, aes(x = Island, y = Percentage)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9), width = 0.7, fill = "#e31a1c") +
  geom_text(data = df_percent_flora_az_long, aes(x = Island, y = ymax + 0.5, 
                                            label =  Total_Species),
            color = "black", size = 4, vjust = -0.5) +
  labs(x = NULL,
       y = "% littoral species") +
...
ggsave(here::here("c_output/figures/plot_percent_litt_islands.png"), height = 15, width = 7)
