...
single_point_min_Galap <- compute_mat_min_weighted(median_min_mat_Galapagos)
single_point_min_Can <- compute_mat_min_weighted(median_min_mat_Canaries)
single_point_min_Az <- compute_mat_min_weighted(median_min_mat_Azores)
...
netplot_indegree_gal <- ggraph(tg, layout = 'manual', x = V(tg)$long, y = V(tg)$lat) + 
  ...
  geom_node_point(aes(color = in_degree), size = 8) +
  scale_edge_width(range = c(0.2, 1), name = "Normalized Connectivity") + # Adjust the range for better visualization
...
ggsave(here::here("c_output/figures/network_indegree_gal.png"), height = 8, width = 9)
...
```{r}
mat_medianMin_sym_Galapagos<-Make_matrix_medianMin_symmetric(single_point_min_Galap)
...
#write.csv(mat_medianMin_sym_Galapagos, "../../Matrices/Galapagos/Matrices-Currents_Galap/matrix_median_min.csv", row.names = FALSE)

#write.csv(mat_medianMin_sym_Canaries, "matrix_median_min_can.csv", row.names = FALSE)

#write.csv(mat_medianMin_sym_Azores, "../../Matrices/Azores/Matrices-Currents_Azores/matrix_median_min.csv", row.names = FALSE)
...
# Assuming your adjacency matrix is named 'single_point_min_Galap'
# Convert the matrix to an igraph object
g <- graph_from_adjacency_matrix(mat_medianMin_sym_Galapagos, mode = "undirected", weighted = TRUE)
...
# Plot the graph using ggraph with spatial coordinates
ggraph(tg, layout = 'manual', x = V(tg)$long, y = V(tg)$lat) + 
  geom_edge_link(aes(width = norm_inv_weight, edge_alpha = norm_inv_weight)) +
  geom_node_point(size = 5, color = 'blue') +
  geom_node_text(aes(label = name), repel = TRUE, size = 5) +
  scale_edge_width(range = c(0.5, 2)) + # Adjust the range for better visualization
  theme_void() + 
...
```{r}
tg_weighted <- tg
E(tg_weighted)$weight <- 1 / E(tg_weighted)$inv_weight  # Adjusting weights based on inverse weights

# Calculate total degree
V(tg_weighted)$degree <- strength(tg_weighted, mode = "all",
                                weights = E(tg)$inv_weight)

# Plot the graph using ggraph with spatial coordinates and color nodes based on total degree
netplot_degree_gal <- ggraph(tg_weighted, layout = 'manual', x = V(tg)$long, y = V(tg)$lat) + 
  geom_edge_link(aes(width = norm_inv_weight), arrow = arrow(length = unit(3, 'mm')), end_cap = circle(3, 'mm'), show.legend = FALSE) +
  ...
  guides(color = guide_colorbar(barwidth = 8, barheight = 1,
                                ...
                                title.hjust = 0.5,
                                ticks = FALSE,
                                breaks = quantile(V(tg_weighted)$degree, probs = c(0, 0.5, 1)))) + # Customize legend breaks and labels
  theme(legend.position = "bottom") +
  my_theme

netplot_degree_gal

ggsave(here::here("c_output/figures/network_wdegree_undirected_gal.png"), height = 8, width = 9)
```
